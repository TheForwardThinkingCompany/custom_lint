...
String _computePubspec(
  ...
  final dependencyOverrides = allDependencies.entries.map((dep) => '  ${dep.key}:\n    path: ${dep.value.root.toFilePath()}').join('\n');
...
class CustomLintWorkspace {
  ...
  Map<String, Package> _computeDependencies() {
    ...
    final result = <String, Package>{
      for (final package in visitPluginsAndDependencies()) package.name: package,
    };
  ...
  Future<Directory> createPluginHostDirectory() async {
    ...
    await Future.wait([
      pubspecFile.create(recursive: true).then((_) => pubspecFile.writeAsString(pubspecContent)),
      Future(() async {
...
class CustomLintPluginCheckerCache {
  ...
  Future<bool> isPlugin(Directory directory) {
    ...
    return _cache[directory] = Future(() async {
      ...
      return pubspec.dependencies.containsKey('custom_lint');
    });
  }
}
    ...
    final conflictingEntriesByPackages = Map.fromEntries(
      _packagesWithConflictingVersions.map(
        (packageName) => MapEntry(
          packageName,
          _entries.where((entry) => entry.packageMeta.package.name == packageName).sortedBy((value) => value.project.pubspec.name),
        ),
      ),
    );
...
''');
    ...
    for (final conflictingEntriesForPackage in conflictingEntriesByPackages.values) {
    ...
    final conflictingEntriesByProjects = <CustomLintProject, List<_ConflictEntry>>{};
    for (final conflictingEntriesForPackage in conflictingEntriesByPackages.values) {
      for (final conflictEntry in conflictingEntriesForPackage) {
        final project = conflictEntry.project;
        final projectConflictingEntries = conflictingEntriesByProjects[project] ?? [];
    ...
    // Build fix commands
    for (final conflictingEntriesForProject in conflictingEntriesByProjects.entries) {
      ...
      final conflictingPackagesNames = conflictingPackageEntries.map((entry) => entry.packageMeta.package.name).join(' ');
...
extension on Pubspec {
  Dependency? getDependency(String name) {
    return dependencies[name] ?? devDependencies[name] ?? dependencyOverrides[name];
  }
}
