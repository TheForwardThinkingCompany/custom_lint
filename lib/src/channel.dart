...
class StreamToSentPortAdapter {
  ...
  StreamToSentPortAdapter(
    ...
    final inputSendport = outputStream.where((e) => e is SendPort).cast<SendPort>().safeFirst;

    final sub = outputStream.where((e) => e is! SendPort).map((e) => e! as Map<String, Object?>).listen((e) => output(e));
...
class _SocketCustomLintClientChannel extends CustomLintClientChannel {
  ...
  @override
  late final Stream<CustomLintRequest> input = socket.messages.cast<Map<String, Object?>>().map(CustomLintRequest.fromJson).asBroadcastStream();
