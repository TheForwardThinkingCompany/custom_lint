...
import 'channel.dart';
// ignore: implementation_imports
import 'core/change_reporter.dart';
import 'core/node_lint_visitor.dart';
...
class CustomLintPluginClient {
  ...
  bool _isPluginActiveForContextRoot(
    ...
    return contextRootsForPlugin.any(
      (contextRootForPlugin) => analysisContext.contextRoot.root == contextRootForPlugin.root,
    );
  }
  ...
  Future<void> _updateActivePluginList(
    ...
    for (final analysisContext in analysisContextCollection.contexts) {
      ...
      for (final pluginName in _channel.registeredPlugins.keys) {
        final isPluginEnabledInContext = pubspec.dependencies.containsKey(pluginName) ||
...
class _CustomLintAnalysisConfigs {
  ...
  factory _CustomLintAnalysisConfigs.from(
    ...
    final configs = CustomLintConfigs.parse(analysisContext.contextRoot.optionsFile);

    final activePluginsForContext = Map.fromEntries(
      client._channel.registeredPlugins.entries.where(
        (MapEntry<String, PluginBase> plugin) => client._isPluginActiveForContextRoot(
          analysisContext,
    ...
    final List<LintRule> rules = _lintRulesForContext(activePluginsForContext, configs);
    final fixes = _fixesForRules(rules);
    final assists = _assistsForContext(activePluginsForContext, configs, client);
  ...
  static List<Assist> _assistsForContext(
    ...
    return activePluginsForContext.entries.expand((plugin) => plugin.value.getAssists()).toList();
  }
...
class _AnalysisErrorsKey {
  ...
  @override
  bool operator ==(Object other) => other is _AnalysisErrorsKey && other.filePath == filePath && other.analysisContext == analysisContext;
...
class _ClientAnalyzerPlugin extends analyzer_plugin.ServerPlugin {
  ...
  var _customLintConfigsForAnalysisContexts = <AnalysisContext, _CustomLintAnalysisConfigs>{};
  final _analysisErrorsForAnalysisContexts = <_AnalysisErrorsKey, Set<AnalysisError>>{};
  ...
  Future<void> afterNewContextCollection({
    ...
    return _runOperation(() async {
      ...
      _analysisErrorsForAnalysisContexts.removeWhere(
        (key, value) => contextCollection.contexts.contains(key.analysisContext),
      );
  ...
  Future<analyzer_plugin.EditGetAssistsResult> handleEditGetAssists(
    ...
    final assists = _customLintConfigsForAnalysisContexts[analysisContext]?.assists;
  ...
  Future<analyzer_plugin.EditGetFixesResult> handleEditGetFixes(
    ...
    final analysisErrorsForContext = _analysisErrorsForAnalysisContexts[key] ?? const {};
    final errorsAtOffset = analysisErrorsForContext
        .where(
          (error) => parameters.offset >= error.offset && parameters.offset <= error.offset + error.length,
        )
        .toList();
  ...
  Future<analyzer_plugin.AnalysisErrorFixes?> _handlesFixesForError(
    ...
    final otherErrors = allErrors
        .where(
          (element) => element != analysisError && element.errorCode == analysisError.errorCode,
        )
        .toList();
  ...
  @override
  Future<analyzer_plugin.AnalysisHandleWatchEventsResult> handleAnalysisHandleWatchEvents(
  ...
  bool _ownsPath(String path) {
    for (final contextRoot in _client._contextRootsForPlugin.values.expand((e) => e)) {
      if (isWithin(contextRoot.root.path, path)) {
        final isExcluded = contextRoot.excludedPaths.any((excludedPath) => isWithin(excludedPath, path));
  ...
  Future<void> analyzeFile({
    ...
    final resolver = analysisContext.createResolverForFile(resourceProvider.getFile(path));
    ...
    await _runOperation(() async {
      ...
      final key = _AnalysisErrorsKey(filePath: path, analysisContext: analysisContext);
  ...
  void _handleGetLintsError(
    ...
    final endOffset = resolver.lineInfo.lineStarts.elementAtOrNull(1) ?? startOffset;
