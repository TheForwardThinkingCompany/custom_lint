...
Future<int> _findPossiblyUnusedPort() {
  return SocketCustomLintServerToClientChannel._createServerSocket().then((value) => value.port);
}
...
class SocketCustomLintServerToClientChannel {
  ...
  late final Stream<CustomLintMessage> _messages = _channel.messages.map((e) => e! as Map<String, Object?>).map(CustomLintMessage.fromJson);

  late final Stream<CustomLintResponse> _responses =
      _messages.where((msg) => msg is CustomLintMessageResponse).cast<CustomLintMessageResponse>().map((e) => e.response);

  /// The events sent by the client.
  late final Stream<CustomLintEvent> events =
      _messages.where((msg) => msg is CustomLintMessageEvent).cast<CustomLintMessageEvent>().map((eventMsg) => eventMsg.event);
  ...
  Future<Process?> _startProcess() async {
    final tempDirectory = _tempDirectory = await _workspace.createPluginHostDirectory();
    ...
    return _asyncRetry(retryCount: 5, () async {
      ...
      final process = await Process.start(
        ...
        ['--enable-vm-service=${await port}', join('lib', 'custom_lint_client.dart'), _serverSocket.port.toString()],
...
import 'dart:convert';
import 'dart:io';
import 'package:custom_lint/src/channel.dart';
$imports
