...
class CustomLintServer {
  ...
  void _start(SendPort sendPort) {
    ...
    _requestSubscription = _analyzerPluginClientChannel.messages.map((e) => e! as Map<String, Object?>).map(Request.fromJson).listen(_handleRequest);
  }
    ...
    return _closeFuture = Future(() async {
      ...
      try {
        await Future.wait([
          ...
          if (_clientChannelEventsSubscription != null) _clientChannelEventsSubscription!.cancel(),
        ])
  ...
  Future<void> _handleEvent(CustomLintEvent event) => _runner.run(() async {
        await event.map(
          analyzerPluginNotification: (event) async {
            await _analyzerPluginClientChannel.sendJson(event.notification.toJson());

            final notification = event.notification;
            if (notification.event == PLUGIN_NOTIFICATION_ERROR) {
              final error = PluginErrorParams.fromNotification(notification);
              await _analyzerPluginClientChannel.sendJson(error.toNotification().toJson());
