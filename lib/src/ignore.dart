...
class IgnoreMetadata {
  ...
  factory IgnoreMetadata._parse(
    ...
    final codes = fullMatchString.substring(fullMatchString.indexOf(':') + 1).split(',').map((e) => e.trim()).toSet();
...
List<IgnoreMetadata> parseIgnoreForFile(String source) {
  ...
  return ignoreForFiles.map((e) => IgnoreMetadata._parse(e, startOffset: 0)).toList();
}
  ...
  void run(
    ...
    ignoreForLineChangeBuilder.addDartFileEdit((builder) {
      if (ignoreForLine.hasIgnore) {
        ...
        final offsetLine = resolver.lineInfo.getLocation(analysisError.offset).lineNumber - 1;
